...
import { PaperProvider, Card, Title, Paragraph, Divider, Text } from 'react-native-paper';
    ...
    <View style={styles.container}>
      <PaperProvider>
        <FlatList
          data={times}
          renderItem={({ item }) => (
            <Card style={{ marginBottom: 10, backgroundColor: 'purple' }}>
              <Card.Content>
                <Title style={{ color: 'white' }}>{item.nome}</Title>
                <Paragraph style={{ color: 'white' }}>{item.anoFundacao}</Paragraph>
                <Paragraph style={{ color: 'white' }}>{item.mascote}</Paragraph>
                <Divider style={{ marginVertical: 10 }} />
                <Text style={{ color: 'white' }}>Jogadores:</Text>
                <FlatList
                  data={item.jogadores}
                  renderItem={({ item: jogador }) => (
                    <View style={styles.jogadorContainer}>
                      ...
                      <View>
                        <Text style={{ color: 'white' }}>{jogador.nome}</Text>
                        <Text style={{ color: 'white' }}>Número: {jogador.numero}</Text>
                      </View>
                    </View>
                  )}
                />
